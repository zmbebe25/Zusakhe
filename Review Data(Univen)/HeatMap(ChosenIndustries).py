...
# Extract industries by CombinedTimeMinutes categories and their counts
categories = [
...
for entry in grouped_data:
    ...
    industries = entry.get("ChosenIndustries", []) or []  # Handle NoneType by defaulting to an empty list

    for category in categories:
        if category == "CombinedTimeMinutes < 1" and combined_time < 1:
            for industry in industries:
                heatmap_data[category][industry] = heatmap_data[category].get(industry, 0) + 1
...
# Limit to top 30 chosen industries by total counts
...
for category in impactful_categories:
    ...
    plt.ylabel("Chosen Industries", fontsize=12)
