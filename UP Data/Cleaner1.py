...
modules = []
with open('UP Data/Bachelor of Education (Intermediate Phase Teaching).txt', 'r', encoding='utf-8') as file:
    data = file.readlines()

for i in range(1, len(data)):  # Start from 1 since we look back at i-1
    line = data[i].strip()
    if line.startswith('Module credits'):
        name = data[i-1].strip()  # Get the name from the previous line
        modules.append(name)

with open('UP Data/Bachelor of Education (Intermediate Phase Teaching).txt', 'w', encoding='utf-8') as file:
