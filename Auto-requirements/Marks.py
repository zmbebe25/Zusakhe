...
input_file_path = 'C:/Users/Zusakhe Mbebe/course-data/Zusakhe/Auto-requirements/Page67(Final).json'
output_file_path = 'C:/Users/Zusakhe Mbebe/course-data/Zusakhe/Auto-requirements/Page67(Final1).json'
...
for entry in data:
    ...
    processed_entry["Physical Sciences"] = extract_numeric_value(entry.get("Physical Sciences", "").replace('\n', ''))
    processed_entry["Life Sciences"] = extract_numeric_value(entry.get("Life Sciences", "").replace('\n', ''))
    processed_entry["Technical Mathematics"] = extract_numeric_value(entry.get("Technical Mathematics", "").replace('\n', ''))
    processed_entry["Technical Science"] = extract_numeric_value(entry.get("Technical Science", "").replace('\n', ''))
